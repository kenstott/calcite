...
public class FileConversionManagerTest {
  ...
  @Test void testRequiresConversionForExcel() {
  ...
  @Test void testRequiresConversionForHtml() {
  ...
  @Test void testRequiresConversionForXml() {
    assertTrue(FileConversionManager.requiresConversion("data.xml"));
    assertTrue(FileConversionManager.requiresConversion("DATA.XML"));
  }

  @Test void testRequiresConversionForMarkdown() {
  ...
  @Test void testRequiresConversionForDocx() {
  ...
  @Test void testRequiresConversionForPptx() {
  ...
  @Test void testConvertIfNeededForCsvReturnsfalse() {
    ...
    boolean converted =
        FileConversionManager.convertIfNeeded(csvFile, tempDir.toFile(), "SMART_CASING");
    assertFalse(converted, "CSV files should not need conversion");
  }

  @Test void testConvertIfNeededForParquetReturnsFalse() {
    ...
    boolean converted =
        FileConversionManager.convertIfNeeded(parquetFile, tempDir.toFile(), "SMART_CASING");
    assertFalse(converted, "Parquet files should not need conversion");
  }

  @Test void testConvertIfNeededFourArgOverload() {
    File csvFile = tempDir.resolve("data.csv").toFile();
    try (FileWriter writer = new FileWriter(csvFile)) {
      writer.write("id,name\n1,test\n");
    } catch (IOException e) {
    ...
    boolean converted =
        FileConversionManager.convertIfNeeded(csvFile, tempDir.toFile(), "SMART_CASING", "SMART_CASING");
    assertFalse(converted, "CSV files should not need conversion (4-arg overload)");
  }

  @Test void testConvertIfNeededFiveArgOverload() {
    File csvFile = tempDir.resolve("data.csv").toFile();
    try (FileWriter writer = new FileWriter(csvFile)) {
      writer.write("id,name\n1,test\n");
    } catch (IOException e) {
    ...
    boolean converted =
        FileConversionManager.convertIfNeeded(csvFile, tempDir.toFile(), "SMART_CASING", "SMART_CASING", tempDir.toFile());
    assertFalse(converted, "CSV files should not need conversion (5-arg overload)");
  }

  @Test void testConvertIfNeededForJsonWithNoConversionRecord() {
    ...
    boolean converted =
        FileConversionManager.convertIfNeeded(jsonFile, tempDir.toFile(), "SMART_CASING", "SMART_CASING",
        tempDir.toFile(), null);
    assertFalse(converted, "JSON file without JSONPath record should not be converted");
  }

  @Test void testConvertIfNeededForYamlWithNoConversionRecord() {
    ...
    boolean converted =
        FileConversionManager.convertIfNeeded(yamlFile, tempDir.toFile(), "SMART_CASING", "SMART_CASING",
        tempDir.toFile(), null);
    assertFalse(converted, "YAML file without JSONPath record should not be converted");
  }

  @Test void testConvertIfNeededForYmlExtension() {
    ...
    boolean converted =
        FileConversionManager.convertIfNeeded(ymlFile, tempDir.toFile(), "SMART_CASING", "SMART_CASING",
        tempDir.toFile(), null);
    assertFalse(converted, "YML file without JSONPath record should not be converted");
  }

  @Test void testConvertIfNeededForJsonGzWithNoConversionRecord() {
    ...
    boolean converted =
        FileConversionManager.convertIfNeeded(jsonGzFile, tempDir.toFile(), "SMART_CASING", "SMART_CASING",
        tempDir.toFile(), null);
    assertFalse(converted, "json.gz file without JSONPath record should not be converted");
  }

  @Test void testConvertIfNeededSkipsWhenFileNotChanged() throws IOException {
    // Create a CSV file
    ...
    ConversionMetadata.ConversionRecord record =
        new ConversionMetadata.ConversionRecord(csvFile.getAbsolutePath(),
    ...
    boolean converted =
        FileConversionManager.convertIfNeeded(csvFile, tempDir.toFile(), "SMART_CASING", "SMART_CASING", baseDir, null);
  ...
  @Test void testFindOriginalSourceWithNoMetadata() {
  ...
  @Test void testFindOriginalSourceWithNonExistentFile() {
  ...
  @Test void testRegisterAndConvert() throws IOException {
    ...
    List<String> result =
        manager.convert(sourceFile, tempDir.toFile(), "test_src", "test_tgt");

    assertNotNull(result);
    assertFalse(result.isEmpty(), "Should produce at least one output path");
  }

  @Test void testConvertWithNoRegisteredConverter() {
    FileConversionManager manager = FileConversionManager.getInstance();
  ...
  @Test void testConvertWithCachedConverter() throws IOException {
    FileConversionManager manager = FileConversionManager.getInstance();
    ...
    List<String> result1 =
        manager.convert(sourceFile, tempDir.toFile(), "cached_src", "cached_tgt");
  ...
  @Test void testConvertIfNeededHandlesExceptionInConversion() throws IOException {
  ...
  @Test void testConvertIfNeededForMarkdownFile() throws IOException {
    ...
    boolean result =
        FileConversionManager.convertIfNeeded(mdFile, tempDir.toFile(), "SMART_CASING", "SMART_CASING",
        null, null);
  ...
  @Test void testExtractJsonPathInternal() throws Exception {
  ...
  @Test void testExtractJsonPathWithNestedBrackets() throws Exception {
    java.lang.reflect.Method method =
        FileConversionManager.class.getDeclaredMethod("extractJsonPath", String.class);
    method.setAccessible(true);
  ...
  @Test void testIsRemoteFileInternal() throws Exception {
    java.lang.reflect.Method method =
        FileConversionManager.class.getDeclaredMethod("isRemoteFile", String.class);
    method.setAccessible(true);
  ...
  @Test void testConvertIfNeededForHtmlWithExistingRecord() throws IOException {
    // Create an HTML file
    ...
    ConversionMetadata metadata = new ConversionMetadata(baseDir);
    ConversionMetadata.ConversionRecord record =
        new ConversionMetadata.ConversionRecord(htmlFile.getAbsolutePath(),
  ...
  @Test void testConvertIfNeededForXmlFile() throws IOException {
    ...
    boolean result =
        FileConversionManager.convertIfNeeded(xmlFile, tempDir.toFile(), "SMART_CASING", "SMART_CASING",
        null, null);
