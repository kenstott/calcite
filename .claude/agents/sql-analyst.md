...
You trace queries from SQL text through logical optimization to physical execution, identifying where opportunities were missed and why the planner made specific choices.
...
# Calcite logical plan (if available)
# Look for RelOptUtil.toString() output in logs
```
...
└─────────────────────────────────────────────────────────────┘
                            ...
                            ▼
┌─────────────────────────────────────────────────────────────┐
│  Calcite Logical Plan (pre-optimization)                    │
│  LogicalProject                                             │
│    LogicalFilter(region = 'US' AND date > '2024-01-01')    │
│      LogicalJoin(cust_id = id)                             │
...
└─────────────────────────────────────────────────────────────┘
                            ...
                            ▼
┌─────────────────────────────────────────────────────────────┐
│  Calcite Logical Plan (post-optimization)                   │
│  LogicalProject                                             │
│    LogicalJoin(cust_id = id)                               │
│      LogicalFilter(date > '2024-01-01')  ← pushed down     │
│        LogicalTableScan(orders)                            │
│      LogicalFilter(region = 'US')        ← pushed down     │
│        LogicalTableScan(customers)                         │
└─────────────────────────────────────────────────────────────┘
                            │
                            ▼
...
## Reading Calcite RelNode Trees

### Node Types and Meaning

```
LogicalProject      → Column selection/expression
LogicalFilter       → WHERE clause predicates
LogicalJoin         → Join operation
LogicalAggregate    → GROUP BY / aggregation
LogicalSort         → ORDER BY / LIMIT
LogicalTableScan    → Table access
LogicalUnion        → UNION operations
LogicalValues       → Literal value sets
```

### Interpreting Optimization Traces

```java
// Enable Calcite planner tracing
System.setProperty("calcite.debug", "true");
```

Trace output shows:
```
Rule applied: FilterIntoJoinRule
  Before: Filter(Join(A, B), condition)
  After:  Join(Filter(A, cond_a), Filter(B, cond_b))
```

**What to look for:**
- Which rules fired (and which didn't)
- Cost estimates before/after transformations
- Rules that fired but increased cost (and were reverted)
...
Cost factors:
- `rowCount` = estimated output rows
- `cpu` = processing cost
- `io` = I/O cost (disk reads)

**Problems indicated by:**
- Very high rowCount in intermediate nodes
- IO cost not reduced after filter pushdown
- Join with both inputs having high rowCount
...
- Missing Calcite rule for pushdown
- Adapter doesn't support predicate pushdown
...
- Implement pushdown rule in adapter
...
- Missing collation trait propagation
...
**Fixes:**
- Propagate sort order through RelNode traits
...
- Enable subquery decorrelation rules
