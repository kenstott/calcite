...
- **Status:** FILE-055 kept **in-progress**. The exact token set, case-insensitive recognition, and the actual binary nullable rule are asserted now; the threshold clause is staged `@Disabled("C-32")` in `CsvInferenceRequirementsTest#columnNullableOnlyWhenNullRatioExceedsThreshold_target`. Resolution pending: either wire `nullableThreshold` into `determineType`, or reword the requirement to the binary rule (the threshold field is then dead and should be removed).
