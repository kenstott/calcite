...
abstract class AbstractArcGisFeatureTransformer implements StreamingResponseTransformer {
  ...
  private static final int MAX_PAGE_PARSE_RETRIES = 3;
  ...
  private final class PageIterator implements Iterator<Map<String, Object>> {
    ...
    private List<JsonNode> fetchPage(int off) throws IOException {
      ...
      JsonNode root = fetchPageJson(off);
    ...
    private JsonNode fetchPageJson(int off) throws IOException {
      ...
      for (int attempt = 1; attempt <= MAX_PAGE_PARSE_RETRIES; attempt++) {
        String body = get(baseUrl + "&resultOffset=" + off);
        try {
          return MAPPER.readTree(body);
        } catch (com.fasterxml.jackson.core.JsonProcessingException e) {
          lastFailure = new IOException(getClass().getSimpleName() + ": malformed JSON at offset "
              + off + " (attempt " + attempt + "/" + MAX_PAGE_PARSE_RETRIES + ", body length "
              + body.length() + " chars) — likely a truncated response", e);
          logger.warn(lastFailure.getMessage());
          if (attempt < MAX_PAGE_PARSE_RETRIES) {
