...
/**
 * DataProvider for {@code usaspending_by_county} — federal spending by
 * place-of-performance county for a fiscal year, from USAspending
 * {@code POST /api/v2/search/spending_by_geography/} ({@code geo_layer=county},
 * all award types). The federal fiscal year is expressed as an Oct-1 .. Sep-30
 * {@code time_period}. The county layer returns ~2,800 rows in a single
 * unpaginated response, the same shape as the state layer.
 *
 ...
 */
public class UsaSpendingCountyProvider implements CachingDataProvider {

  private static final Logger LOGGER = LoggerFactory.getLogger(UsaSpendingCountyProvider.class);
  ...
  /** Contracts, IDVs, grants, direct payments, loans, and other — matches by_state. */
  ...
  @Override public Iterator<Map<String, Object>> fetch(EtlPipelineConfig config,
    ...
    if (year == null || year.isEmpty()) {
      LOGGER.warn("usaspending_by_county: no year in dimension variables {}", variables);
    ...
    LOGGER.info("usaspending_by_county: POST {} fy={}", ENDPOINT, fy);
    ...
    LOGGER.info("usaspending_by_county: {} county rows for fy {}", rows.size(), fy);
