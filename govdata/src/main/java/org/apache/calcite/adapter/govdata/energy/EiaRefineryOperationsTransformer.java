    ...
    try {
      ...
      for (JsonNode row : data) {
        ...
        try {
          ...
          rowsSkipped++;
          LOGGER.warn("EIA Refinery Operations: skipping row with unparseable period '{}'", period);
          continue;
        }
        ObjectNode out = MAPPER.createObjectNode();

        out.put("report_year", reportYear);
      ...
      if (rowsSkipped > 0) {
        LOGGER.warn("EIA Refinery Operations: skipped {} row(s) with unparseable period", rowsSkipped);
      }
