...
/**
 * Streaming transformer for the USDA RMA "Cause of Loss — Summary of Business
 * with Month of Loss (1989 Forward)" bulk files.
 *
 * <p>Each {@code colsom_{year}.zip} holds one pipe-delimited, header-less text file
 * with 30 fixed fields (documented in RMA's record layout at
 * {@code COL_Summary_of_Business_with_Month_All_Years.pdf}), one row per State ×
 * County × Crop × Insurance Plan × Coverage Category × Stage × Cause of Loss ×
 * Month of Loss. This is a materially finer grain than
 * {@link RmaSummaryOfBusinessTransformer}'s state/county/crop file, which has no
 * cause-of-loss dimension at all. Text fields are space-padded to fixed width and
 * are trimmed here; a 5-digit {@code county_fips} is derived (state + county) for
 * the geo FK.
 *
 * <p>Implemented as a {@link StreamingResponseTransformer} because the source is a
 * multi-megabyte binary zip (millions of rows for a single crop year) that must
 * not be buffered as a String; rows are produced lazily.
 */
public class RmaCauseOfLossTransformer implements StreamingResponseTransformer {

  private static final Logger LOGGER = LoggerFactory.getLogger(RmaCauseOfLossTransformer.class);
  ...
  private static final String[][] FIELDS = {
      ...
      {"coverage_category", "S"},
      {"stage_code", "S"},
      {"cause_of_loss_code", "S"},
      {"cause_of_loss_desc", "S"},
      {"month_of_loss", "I"},
      {"month_of_loss_name", "S"},
      {"year_of_loss", "I"},
      {"policies_earning_prem_count", "L"},
      {"policies_indemnified_count", "L"},
      {"net_planted_quantity", "D"},
      {"net_endorsed_acres", "D"},
      {"liability_amount", "D"},
      {"total_premium_amount", "D"},
      {"producer_paid_premium_amount", "D"},
      {"subsidy_amount", "D"},
      {"state_private_subsidy", "D"},
      {"additional_subsidy", "D"},
      {"efa_premium_discount", "D"},
      {"net_determined_quantity", "D"},
      {"indemnity_amount", "D"},
      {"loss_ratio", "D"},
  };
    ...
    return new Iterator<Map<String, Object>>() {
      ...
      private void fill() {
        try {
          ...
          throw new RuntimeException("Failed streaming RMA Cause of Loss file: " + url, e);
        }
  ...
  private static InputStream openFirstZipEntry(String url) throws IOException {
    ...
    if (code < 200 || code >= 300) {
      throw new IOException("RMA Cause of Loss download HTTP " + code + ": " + url);
    }
    ZipInputStream zis = new ZipInputStream(conn.getInputStream());
    if (zis.getNextEntry() == null) {
      zis.close();
      throw new IOException("RMA Cause of Loss zip is empty: " + url);
    }
    LOGGER.debug("RMA: streaming Cause of Loss file from {}", url);
    return zis;
  }
}
