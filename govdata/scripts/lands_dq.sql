...
SELECT 'lands', 'forest_metrics', 'T1_existence',
  CASE WHEN n > 0 THEN 'pass' ELSE 'fail' END,
  n, 1, 'Row count from iceberg_scan'
FROM (SELECT COUNT(*) AS n FROM iceberg_scan('s3://govdata-parquet-v1/lands/forest_metrics', allow_moved_paths := true));
...
-- T3: sample
SELECT * FROM iceberg_scan('s3://govdata-parquet-v1/lands/forest_metrics', allow_moved_paths := true) LIMIT 3;
...
FROM (
  ...
  FROM (
    SELECT column_name, null_percentage
    FROM (SUMMARIZE SELECT * FROM iceberg_scan('s3://govdata-parquet-v1/lands/forest_metrics', allow_moved_paths := true))
    WHERE null_percentage = 100.0
      AND column_name NOT IN ('type', 'stateAbbr')
  )
);
...
SELECT 'lands', 'forest_metrics', 'T6_state_coverage',
  CASE WHEN n >= 40 THEN 'pass' ELSE 'fail' END,
  n, 40, 'Distinct state_fips values'
FROM (SELECT COUNT(DISTINCT state_fips) AS n FROM iceberg_scan('s3://govdata-parquet-v1/lands/forest_metrics', allow_moved_paths := true));
...
FROM (
  SELECT COUNT(*) AS bad
  FROM iceberg_scan('s3://govdata-parquet-v1/lands/forest_metrics', allow_moved_paths := true)
...
FROM (
  ...
  FROM iceberg_scan('s3://govdata-parquet-v1/lands/forest_metrics', allow_moved_paths := true) fm
  LEFT JOIN iceberg_scan('s3://govdata-parquet-v1/lands/forest_inventory', allow_moved_paths := true) fi
