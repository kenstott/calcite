...
# Pin the jar for the whole pool: resolve_classpath returns an immutable copy, and every worker
# inherits it through GOVDATA_JAR, so a jar staged mid-run never reaches this pool. The next pool
# to start picks up the new one.
POOL_JAR="$(resolve_classpath)"
export GOVDATA_JAR="$POOL_JAR"
...
fill_pool() {
  ...
  while [ "${#active_pids[@]}" -lt "$MAX_WORKERS" ] && [ "$scan_idx" -lt "$total" ]; do
    ...
    # for as long as the conflict lasts. Requeuing at the back gets both: this slot is guaranteed
    # to be reattempted (it's still in the queue, `total` grows to cover it, same as a
