...
if [ $# -eq 0 ]; then
  ...
  echo "  $0 all                    — all historical/initial workers (1-41, 60-62, 67, 71)"
...
for arg in "$@"; do
  ...
  for part in "${parts[@]}"; do
    if [ "$part" = "all" ]; then
      ...
      # and 72-73 (edu annual/biennial). Run those explicitly or via cron.
      for i in $(seq 1 41); do queue+=("$i"); done
      for i in 60 61 62 67 71; do queue+=("$i"); done
