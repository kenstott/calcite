#!/usr/bin/env bash
# Perpetual scheduled pool runner — alternates historical/daily in 12-hour windows.
# Auto-restarts the pool on non-zero exit (OOM / crash) within the current window.
# Errors (crash restarts) are written to errors.log in addition to the per-window log.
#
# Usage: run-scheduled.sh [historical|daily]
#   Optional first argument: which mode to start with.
#   Default: determined by hour of day (8AM-8PM = daily, 8PM-8AM = historical).
#   Runs forever until SIGTERM / SIGINT.
#
# This script is self-cycling — install it as a single persistent systemd service
# rather than two timers.  See systemd/install.sh for the unit definition.
