#!/usr/bin/env bash
# monitor-pipeline.sh — Report materialized output counts from Iceberg tables (govdata-parquet-v1)
# plus in-flight staging counts from the tracker (govdata-tracker-v1, metadata-only for uncompacted years).
