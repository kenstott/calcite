...
SELECT 'cyber_vuln', 'cwe_catalog', 'existence',
  ...
  n, 1, 'row count'
FROM (SELECT COUNT(*) AS n FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/cwe_catalog', allow_moved_paths := true));
...
-- T3: sample
SELECT * FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/cwe_catalog', allow_moved_paths := true) LIMIT 3;
...
FROM (
  SELECT column_name
  FROM (SUMMARIZE SELECT * FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/cwe_catalog', allow_moved_paths := true))
...
FROM (
  SELECT COUNT(*) AS n
  FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/cwe_catalog', allow_moved_paths := true)
...
FROM (SELECT COUNT(DISTINCT abstraction) AS n FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/cwe_catalog', allow_moved_paths := true));
...
-- T3: sample
SELECT * FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/vulnerabilities', allow_moved_paths := true) LIMIT 3;
...
FROM (
  SELECT column_name
  FROM (SUMMARIZE SELECT * FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/vulnerabilities', allow_moved_paths := true))
  WHERE null_percentage = 100.0
);
...
FROM (
  SELECT COUNT(*) AS n
  FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/vulnerabilities', allow_moved_paths := true)
...
-- T3: sample
SELECT * FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/vulnerability_cwes', allow_moved_paths := true) LIMIT 3;
...
FROM (
  SELECT column_name
  FROM (SUMMARIZE SELECT * FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/vulnerability_cwes', allow_moved_paths := true))
  WHERE null_percentage = 100.0
);
...
FROM (
  SELECT COUNT(*) AS n
  FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/vulnerability_cwes', allow_moved_paths := true)
...
SELECT 'cyber_vuln', 'kev_catalog', 'existence',
  CASE WHEN n = 0 THEN 'fail' ELSE 'pass' END,
  n, 1, 'row count'
FROM (SELECT COUNT(*) AS n FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/kev_catalog', allow_moved_paths := true));
...
-- T3: sample
SELECT * FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/kev_catalog', allow_moved_paths := true) LIMIT 3;
...
FROM (
  SELECT column_name
  FROM (SUMMARIZE SELECT * FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/kev_catalog', allow_moved_paths := true))
  WHERE null_percentage = 100.0
);
...
FROM (
  SELECT COUNT(*) AS n
  FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/kev_catalog', allow_moved_paths := true)
  WHERE cve_id IS NULL
);
...
SELECT 'cyber_vuln', 'kev_cwes', 'existence',
  CASE WHEN n = 0 THEN 'fail' ELSE 'pass' END,
  n, 1, 'row count'
FROM (SELECT COUNT(*) AS n FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/kev_cwes', allow_moved_paths := true));
...
-- T3: sample
SELECT * FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/kev_cwes', allow_moved_paths := true) LIMIT 3;
...
FROM (
  SELECT column_name
  FROM (SUMMARIZE SELECT * FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/kev_cwes', allow_moved_paths := true))
  WHERE null_percentage = 100.0
);
...
FROM (
  SELECT COUNT(*) AS n
  FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/kev_cwes', allow_moved_paths := true)
  WHERE cve_id IS NULL OR cwe_id IS NULL
);
...
FROM (
  SELECT COUNT(*) AS n
  FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/kev_cwes', allow_moved_paths := true) kc
  WHERE NOT EXISTS (
    SELECT 1 FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/kev_catalog', allow_moved_paths := true) k
...
-- T3: sample
SELECT * FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/osv_vulnerabilities', allow_moved_paths := true) LIMIT 3;
...
FROM (
  SELECT column_name
  FROM (SUMMARIZE SELECT * FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/osv_vulnerabilities', allow_moved_paths := true))
  WHERE null_percentage = 100.0
);
...
FROM (
  SELECT COUNT(*) AS n
  FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/osv_vulnerabilities', allow_moved_paths := true)
...
FROM (SELECT COUNT(DISTINCT ecosystem) AS n FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/osv_vulnerabilities', allow_moved_paths := true));
...
SELECT 'cyber_vuln', 'vuln_cross_refs', 'existence',
  CASE WHEN n = 0 THEN 'fail' ELSE 'pass' END,
  n, 1, 'row count'
FROM (SELECT COUNT(*) AS n FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/vuln_cross_refs', allow_moved_paths := true));
...
-- T3: sample
SELECT * FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/vuln_cross_refs', allow_moved_paths := true) LIMIT 3;
...
FROM (
  SELECT column_name
  FROM (SUMMARIZE SELECT * FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/vuln_cross_refs', allow_moved_paths := true))
  WHERE null_percentage = 100.0
);
...
FROM (
  SELECT COUNT(*) AS n
  FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/vuln_cross_refs', allow_moved_paths := true)
...
-- T3: sample
SELECT * FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/advisories', allow_moved_paths := true) LIMIT 3;
...
FROM (
  SELECT column_name
  FROM (SUMMARIZE SELECT * FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/advisories', allow_moved_paths := true))
  WHERE null_percentage = 100.0
);
...
FROM (
  SELECT COUNT(*) AS n
  FROM iceberg_scan('s3://${GOVDATA_DQ_BUCKET}/cyber_vuln/cyber_vuln/advisories', allow_moved_paths := true)
