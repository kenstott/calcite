...
-- Tables: 19 partitioned tables (15 CDE + 4 BJS)
-- NOTE: cde_cargo_theft, cde_human_trafficking, cde_participation, cde_estimates
--       use known-broken CDE endpoints (403 as of 2026-02) — T1/T2 use 'warn'.
-- ============================================================================
...
-- ============================================================================
-- cde_cargo_theft (BROKEN endpoint — 403 as of 2026-02; use 'warn' for T1/T2)
-- ============================================================================

-- T1: existence (warn: known-broken endpoint)
INSERT INTO dq_results
SELECT 'crime', 'cde_cargo_theft', 'existence',
  CASE WHEN n = 0 THEN 'warn' ELSE 'pass' END,
  n, 1, 'BROKEN endpoint (403 as of 2026-02) — empty table expected'
FROM (SELECT COUNT(*) AS n FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_cargo_theft', allow_moved_paths := true));

-- T2: row_count (warn: broken endpoint)
INSERT INTO dq_results
SELECT 'crime', 'cde_cargo_theft', 'row_count',
  CASE WHEN n < 5000 THEN 'warn' ELSE 'pass' END,
  n, 5000, 'BROKEN endpoint — expect 0 until CDE migrates cargo-theft route'
FROM (SELECT COUNT(*) AS n FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_cargo_theft', allow_moved_paths := true));

-- T3: sample
SELECT * FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_cargo_theft', allow_moved_paths := true) LIMIT 3;

-- T4: all_null_cols
INSERT INTO dq_results
SELECT 'crime', 'cde_cargo_theft', 'all_null_cols',
  CASE WHEN COUNT(*) > 0 THEN 'fail' ELSE 'pass' END,
  COUNT(*), 0, STRING_AGG(column_name, ', ')
FROM (
  SELECT column_name
  FROM (SUMMARIZE SELECT * FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_cargo_theft', allow_moved_paths := true))
  WHERE null_percentage = 100.0
);

-- T5: all_same_value
INSERT INTO dq_results
SELECT 'crime', 'cde_cargo_theft', 'all_same_value',
  CASE WHEN COUNT(*) > 0 THEN 'warn' ELSE 'pass' END,
  COUNT(*), 0, STRING_AGG(column_name, ', ')
FROM (
  SELECT column_name
...
-- T6: pk_nulls (state_abbr, year NOT NULL)
INSERT INTO dq_results
SELECT 'crime', 'cde_cargo_theft', 'pk_nulls',
  CASE WHEN n > 0 THEN 'fail' ELSE 'pass' END,
  n, 0, 'NULL state_abbr or year'
FROM (
  SELECT COUNT(*) AS n
  FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_cargo_theft', allow_moved_paths := true)
  WHERE state_abbr IS NULL OR year IS NULL
);

-- T7: if data exists, count non-negative
INSERT INTO dq_results
SELECT 'crime', 'cde_cargo_theft', 'expected_values',
  CASE WHEN n > 0 THEN 'warn' ELSE 'pass' END,
  n, 0, 'negative count (or no data yet)'
FROM (
  SELECT COUNT(*) AS n
  FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_cargo_theft', allow_moved_paths := true)
  WHERE count < 0
...
-- ============================================================================
-- cde_human_trafficking (BROKEN endpoint — 403 as of 2026-02; use 'warn')
-- ============================================================================

-- T1: existence (warn: known-broken endpoint)
INSERT INTO dq_results
SELECT 'crime', 'cde_human_trafficking', 'existence',
  CASE WHEN n = 0 THEN 'warn' ELSE 'pass' END,
  n, 1, 'BROKEN endpoint (403 as of 2026-02) — empty table expected'
FROM (SELECT COUNT(*) AS n FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_human_trafficking', allow_moved_paths := true));

-- T2: row_count (warn: broken endpoint)
INSERT INTO dq_results
SELECT 'crime', 'cde_human_trafficking', 'row_count',
  CASE WHEN n < 1000 THEN 'warn' ELSE 'pass' END,
  n, 1000, 'BROKEN endpoint — expect 0 until CDE migrates human-trafficking route'
FROM (SELECT COUNT(*) AS n FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_human_trafficking', allow_moved_paths := true));

-- T3: sample
SELECT * FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_human_trafficking', allow_moved_paths := true) LIMIT 3;

-- T4: all_null_cols
INSERT INTO dq_results
SELECT 'crime', 'cde_human_trafficking', 'all_null_cols',
  CASE WHEN COUNT(*) > 0 THEN 'fail' ELSE 'pass' END,
  COUNT(*), 0, STRING_AGG(column_name, ', ')
FROM (
  SELECT column_name
  FROM (SUMMARIZE SELECT * FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_human_trafficking', allow_moved_paths := true))
  WHERE null_percentage = 100.0
);

-- T5: all_same_value
INSERT INTO dq_results
SELECT 'crime', 'cde_human_trafficking', 'all_same_value',
  CASE WHEN COUNT(*) > 0 THEN 'warn' ELSE 'pass' END,
  COUNT(*), 0, STRING_AGG(column_name, ', ')
FROM (
  SELECT column_name
...
-- T6: pk_nulls (state_abbr, year NOT NULL)
INSERT INTO dq_results
SELECT 'crime', 'cde_human_trafficking', 'pk_nulls',
  CASE WHEN n > 0 THEN 'fail' ELSE 'pass' END,
  n, 0, 'NULL state_abbr or year'
FROM (
  SELECT COUNT(*) AS n
  FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_human_trafficking', allow_moved_paths := true)
  WHERE state_abbr IS NULL OR year IS NULL
);

-- T7: count non-negative if data exists
INSERT INTO dq_results
SELECT 'crime', 'cde_human_trafficking', 'expected_values',
  CASE WHEN n > 0 THEN 'warn' ELSE 'pass' END,
  n, 0, 'negative count (or no data yet)'
FROM (
  SELECT COUNT(*) AS n
...
-- ============================================================================
-- cde_participation (BROKEN endpoint — 403 as of 2026-02; use 'warn')
-- ============================================================================

-- T1: existence (warn: known-broken endpoint)
INSERT INTO dq_results
SELECT 'crime', 'cde_participation', 'existence',
  CASE WHEN n = 0 THEN 'warn' ELSE 'pass' END,
  n, 1, 'BROKEN endpoint (403 as of 2026-02) — empty table expected'
FROM (SELECT COUNT(*) AS n FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_participation', allow_moved_paths := true));

-- T2: row_count (warn: broken endpoint)
INSERT INTO dq_results
SELECT 'crime', 'cde_participation', 'row_count',
  CASE WHEN n < 1000 THEN 'warn' ELSE 'pass' END,
  n, 1000, 'BROKEN endpoint — expect 0 until CDE migrates participation route'
FROM (SELECT COUNT(*) AS n FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_participation', allow_moved_paths := true));

-- T3: sample
SELECT * FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_participation', allow_moved_paths := true) LIMIT 3;

-- T4: all_null_cols
INSERT INTO dq_results
SELECT 'crime', 'cde_participation', 'all_null_cols',
  CASE WHEN COUNT(*) > 0 THEN 'fail' ELSE 'pass' END,
  COUNT(*), 0, STRING_AGG(column_name, ', ')
FROM (
  SELECT column_name
  FROM (SUMMARIZE SELECT * FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_participation', allow_moved_paths := true))
  WHERE null_percentage = 100.0
);

-- T5: all_same_value
INSERT INTO dq_results
SELECT 'crime', 'cde_participation', 'all_same_value',
  CASE WHEN COUNT(*) > 0 THEN 'warn' ELSE 'pass' END,
  COUNT(*), 0, STRING_AGG(column_name, ', ')
FROM (
  SELECT column_name
...
-- T6: pk_nulls (state_abbr, year NOT NULL)
INSERT INTO dq_results
SELECT 'crime', 'cde_participation', 'pk_nulls',
  CASE WHEN n > 0 THEN 'fail' ELSE 'pass' END,
  n, 0, 'NULL state_abbr or year'
FROM (
  SELECT COUNT(*) AS n
  FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_participation', allow_moved_paths := true)
  WHERE state_abbr IS NULL OR year IS NULL
);

-- T7: value non-negative if data exists (participation rates/counts)
INSERT INTO dq_results
SELECT 'crime', 'cde_participation', 'expected_values',
  CASE WHEN n > 0 THEN 'warn' ELSE 'pass' END,
  n, 0, 'negative participation value (or no data yet)'
FROM (
  SELECT COUNT(*) AS n
  FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_participation', allow_moved_paths := true)
  WHERE value < 0
...
-- ============================================================================
-- cde_estimates (BROKEN endpoint — 403 as of 2026-02; use 'warn')
-- ============================================================================

-- T1: existence (warn: known-broken endpoint)
INSERT INTO dq_results
SELECT 'crime', 'cde_estimates', 'existence',
  CASE WHEN n = 0 THEN 'warn' ELSE 'pass' END,
  n, 1, 'BROKEN endpoint (403 as of 2026-02) — empty table expected'
FROM (SELECT COUNT(*) AS n FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_estimates', allow_moved_paths := true));

-- T2: row_count (warn: broken endpoint)
INSERT INTO dq_results
SELECT 'crime', 'cde_estimates', 'row_count',
  CASE WHEN n < 5000 THEN 'warn' ELSE 'pass' END,
  n, 5000, 'BROKEN endpoint — expect 0 until CDE migrates estimates route'
FROM (SELECT COUNT(*) AS n FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_estimates', allow_moved_paths := true));

-- T3: sample
SELECT * FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_estimates', allow_moved_paths := true) LIMIT 3;

-- T4: all_null_cols
INSERT INTO dq_results
SELECT 'crime', 'cde_estimates', 'all_null_cols',
  CASE WHEN COUNT(*) > 0 THEN 'fail' ELSE 'pass' END,
  COUNT(*), 0, STRING_AGG(column_name, ', ')
FROM (
  SELECT column_name
  FROM (SUMMARIZE SELECT * FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_estimates', allow_moved_paths := true))
  WHERE null_percentage = 100.0
);

-- T5: all_same_value
INSERT INTO dq_results
SELECT 'crime', 'cde_estimates', 'all_same_value',
  CASE WHEN COUNT(*) > 0 THEN 'warn' ELSE 'pass' END,
  COUNT(*), 0, STRING_AGG(column_name, ', ')
FROM (
  SELECT column_name
...
-- T6: pk_nulls (state_abbr, year, offense_code NOT NULL)
INSERT INTO dq_results
SELECT 'crime', 'cde_estimates', 'pk_nulls',
  CASE WHEN n > 0 THEN 'fail' ELSE 'pass' END,
  n, 0, 'NULL state_abbr, year, or offense_code'
FROM (
  SELECT COUNT(*) AS n
  FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_estimates', allow_moved_paths := true)
  WHERE state_abbr IS NULL OR year IS NULL OR offense_code IS NULL
);

-- T7: estimated_count non-negative if data exists
INSERT INTO dq_results
SELECT 'crime', 'cde_estimates', 'expected_values',
  CASE WHEN n > 0 THEN 'warn' ELSE 'pass' END,
  n, 0, 'negative estimated_count (or no data yet)'
FROM (
  SELECT COUNT(*) AS n
  FROM iceberg_scan('s3://govdata-parquet-v1/crime/cde_estimates', allow_moved_paths := true)
