...
| cyber_threat | 2026-06-07 | PASS    | 0     | 0     | Refresh config on all 11 tables (dataset_type: snapshot + overwritePartitions; etag freshness on the single-file GET sources). All tables OK with keys configured. Final dq-rebuild: PASS (0 fails, 0 warns), incl ioc_hashes fully populated (629 rows, sha256 not null). Fixes: (1) CSV parser handles quoted/comma-space format (`"a", "b"`) — ioc_hashes 0→629 rows; (2) removed stale ioc_hashes column `source:` remaps (transformer emits final names; materialization reads via getEffectiveSource→name); (3) OTX delta mode now bounds the fetch (was walking all 8,598 subscribed pulses; CYBER_OTX_DELTA_DAYS=30 → 171 in 95s); (4) DQ FAILS (not warns) on a missing-key key-gated table — ioc_mixed/threat_pulses pass with keys; (5) ioc_mixed expected ioc_types += md5_hash/sha256_hash. Clean-bucket dq-rebuild: PASS (0 fails, 0 warns) across ALL 11 tables (66/66 checks). Fixed a freshness-gate bug: the dq-rebuild teardown removes Iceberg metadata but leaves the freshness token, so etag-gated tables saw "unchanged → skip" and never rebuilt the metadata, leaving them unscannable (DuckDB "Could not guess Iceberg table version"). The gate now bypasses the skip when `forceReprocessAll` is set (no committed data), so the table always re-materializes; it still probes + persists the token so the next run skips normally |
| cyber_vuln   | 2026-06-08 | WARN    | 0     | 6     | Clean dq-rebuild (GOVDATA_START_YEAR=2025) with the freshness-gate fix: all 8/8 tables materialize + scan (cwe 969, kev 1612, kev_cwes 1539, osv 20,343, vulnerabilities 30,799, vulnerability_cwes 34,706, vuln_cross_refs 27,623, advisories 20). The 4 freshness-gated tables (cwe/kev/kev_cwes/vuln_cross_refs) re-materialize after teardown instead of skipping into an unscannable table. Row-count thresholds recalibrated to DQ-LEVEL (2025 window), not prod: vulnerabilities 120k→25k, vulnerability_cwes 100k→28k, osv 40k→16k, advisories 20→10. 0 fails; 6 non-fatal warns (all_same_value on a uniform column in the small window; osv single-ecosystem) |
