...
| law | `law:daily` | Not queued (current-release snapshot; no point-in-time history to backfill) | Re-fetches every title from the current OLRC release point (`title` list dimension, 55 partitions) and replaces each `title=` partition; per-title zips are cached per release point, so an unchanged release point re-parses from cache without re-downloading | `usc_sections` (`overwritePartitions: true`) | None | None |
...
| econ         | 2026-06-11 | WARN    | 0     | 3     | **First-ever clean econ DQ run** (0 fails / 3 warns / 63 pass). Fixed 4 fails surfaced on first completion: `jolts_state.state_name` was null-by-design ("join with geo.states") → **populated** via `CASE` on the state FIPS in `series_id` (same mechanism as `state_wages`); `jolts_regional.percent_change_month`/`_year` were dead `expression: "NULL"` cols (not in raw BLS) → **removed**; `metro_industry` row-count floor 10000→7000 (BLS returns HTTP 400 for nonexistent metro×industry series — ~7456 valid rows is the real ceiling). **Op fix**: econ had no `workerTimeoutMinutes` override → fell to the 60-min default, and `regional_income`'s BEA geo fan-out (geo_fips_set STATE/COUNTY/MSA/MIC/PORT/DIV/CSA × ~63 tables × line_code via the BEA API @ ~9/s) runs hours, so the watchdog recycled the worker hourly; set `workerTimeoutMinutes: 1440` → completes in one session. Also added `freshness: last_modified` to the 5 BEA bulk-ZIP tables (SAINC/SAGDP/SQINC/SQGDP/SAPCE — detects BEA benchmark-revision republishes the tracker alone freezes out) and `dqRowLimit: 25000` to county_qcew/county_wages. Tracker bypass for the jolts re-ingest used `GOVDATA_FORCE_REPROCESS_TABLES` (the dq-tracker is un-listable, see [[tracker-bucket-bloat]]). **Engine fix** ([[dimension-empty-zeroes-product]]): `DimensionIterator.expandStandard` dropped an empty dimension instead of zeroing the Cartesian product — a lagged-annual daily pass inverts yearRange to `[2026,2025]`, so county_qcew emitted a `year`-less combo that materialised a broken empty Iceberg table; now empty dimension → 0 combinations (clean skip). dqRowLimit validated: county lands 50k = 25k×2yr. 3 benign warns: `all_same_value` on gdp_statistics.seasonally_adjusted, jolts_regional.region_code/region_name (only US-national `00` in feed). county_wages.state_fips T5-excluded (cap-sample artifact — 25k cap takes first low-FIPS rows) |
...
| law          | —          | PENDING | —     | —     | New schema (`usc_sections`, U.S. Code from OLRC USLM XML). Not yet run in the DQ bucket. Isolated local run (local dirs + DuckDB tracker, 119-111 release point): 60,497 rows, 55/55 batches, peak heap 589 MB; `law_dq.sql` 13/13 pass against that output. |
...
| ag           | —          | PENDING | —     | —     | New USDA schema, 5 tables (nass_crop_production, nass_livestock_inventory, rma_crop_insurance, ers_farm_income, fsa_commodity_payments). Wired into worker.sh / run-pool.sh / common.sh: `ag:historical` (single once slot; ERS is a no-year full-CSV fetch) + `ag:daily`; heap 4g/6g historical (FSA loads full xlsx workbooks + payee-dedup sets) / 2g/3g daily. **No data materialized yet** — `ag_dq.sql` deliberately deferred: the iceberg_scan path (single- vs double-nested) can't be discovered without data, and T7 assertions can't be grounded, so writing it now would be guessing. Also NOT added to the run-pool `dq`/`dq-rebuild` hardcoded aliases to avoid injecting guaranteed failures into shared DQ runs. After the first `ag:historical` ETL: discover the path, author `ag_dq.sql`, add `ag:dq` to the aliases, and tune per-table `releaseWindow`/`incrementalTtlDays`. |
