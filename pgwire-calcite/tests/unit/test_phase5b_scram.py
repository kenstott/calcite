...
def test_scram_end_to_end_with_psql(tmp_path):
    # Resolve psql on PATH. The previous /usr/bin/psql literal missed every
    # non-system install (homebrew, postgres.app, a container image), so this
    # end-to-end SCRAM check silently reported a pass without connecting.
    psql = shutil.which("psql")
    assert psql is not None, "psql must be on PATH to exercise SCRAM end to end"
    ...
    try:
        ...
        out = subprocess.run(
            [psql, f"host=127.0.0.1 port={port} user=alice dbname=calcite",
             "-tAc", "SELECT 1"],
