    ...
    def execute_sql(self, sql: str, params=None) -> CalciteQueryResult:
        ...
        try:
            ...
            result = _state.backend.execute_sql(  # type: ignore[attr-defined]
                stripped, self.role_id, params, stream=True
            )
...
class CalciteHandler(BuenaVistaHandler):  # PGW-002, PGW-007
    ...
    def _send_lockout(self, locked: LockedOut) -> None:
        # No shared post-auth NoticeResponse helper exists on this handler yet (tracked
        # separately); answered as an ErrorResponse with SQLSTATE 28000
        # (invalid_authorization_specification) in the meantime.
