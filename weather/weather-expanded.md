...
## Phase 3 — NOAA Storm Events (Disaster Impact Data)

**Goal:** Historical record of damaging weather events with dollar losses. Enables correlation
with SEC 10-K risk disclosures, crop insurance claims, municipal bond ratings, FEMA disaster
declarations, and insurance company financial results.

### New Table: `storm_events`

**Source:** NOAA NCEI Storm Events Database — `https://www.ncdc.noaa.gov/stormevents/`
Bulk CSV download, one file per year. Annual updates.
**Auth:** None
**Partitioned by:** `year`, `state_fips`
...
| Column | Type | Description |
|---|---|---|
| event_id | BIGINT | NCEI event ID |
| state_fips | VARCHAR | State FIPS |
| county_fips | VARCHAR | County FIPS (derived: state + CZ_FIPS when type=C) |
| year | INTEGER | Event year (partition) |
| begin_date | DATE | Event start date |
| end_date | DATE | Event end date |
| event_type | VARCHAR | Tornado, Flood, Hurricane, Hail, Winter Storm, Wildfire, etc. |
| magnitude | DOUBLE | Wind speed (kts), hail size (in), or flood depth (ft) |
| magnitude_type | VARCHAR | EG (estimated gust), MS (measured), etc. |
| deaths_direct | INTEGER | Fatalities directly caused |
| deaths_indirect | INTEGER | Fatalities indirectly caused |
| injuries_direct | INTEGER | Direct injuries |
| property_damage | BIGINT | Property damage in USD (parsed from K/M/B notation) |
| crop_damage | BIGINT | Crop damage in USD |
| narrative | VARCHAR | Event narrative text |
| cz_type | VARCHAR | C (county), Z (zone), M (marine) |

**Implementation notes:**
- NCEI uses `K`/`M`/`B` suffixes for damage amounts; parse to integer dollars during ETL.
- Filter `cz_type = 'C'` for county-level events; zone-level events require zone→county crosswalk.
- `county_fips = state_fips || LPAD(cz_fips, 3, '0')` for county events.

**Cross-schema research enabled:**
```sql
-- SEC 10-K filings mentioning "natural disaster" in counties hit by storm events
SELECT se.event_type, se.property_damage, fm.company_name,
       ms.section_text
FROM weather.storm_events se
JOIN sec.filing_metadata fm
  ON fm.county_fips = se.county_fips          -- requires HQ county on filing_metadata
  AND fm.filed_date BETWEEN se.begin_date AND se.begin_date + INTERVAL '365' DAY
JOIN sec.mda_sections ms ON ms.cik = fm.cik
WHERE se.property_damage > 1000000
  AND ms.section_text LIKE '%natural disaster%';

-- Crop damage vs. agricultural commodity prices (with worldbank.commodities)
SELECT se.county_fips, se.year, SUM(se.crop_damage) AS total_crop_damage,
       wb.price_usd AS corn_price
FROM weather.storm_events se
JOIN worldbank.commodities wb ON wb.commodity = 'Maize' AND wb.year = se.year
WHERE se.event_type IN ('Drought', 'Flood', 'Hail', 'Tornado')
GROUP BY se.county_fips, se.year, wb.price_usd;
```
...
**Goal:** Fire perimeter records with acreage and cause, plus satellite-detected smoke extent
by county. Enables correlation with timber company SEC filings, respiratory health admissions,
agricultural loss, air quality (joins existing `epa_annual_aqi`), and property insurance.

### New Table: `nifc_fire_incidents`

**Source:** NIFC ICS-209 Incident Reports + InciWeb archive
Alternatively: USFS Fire Occurrence Database (FODB) — annual CSV
**Auth:** None
**Partitioned by:** `year`, `state_fips`
...
| Column | Type | Description |
|---|---|---|
| incident_id | VARCHAR | NIFC/ICS incident ID |
| incident_name | VARCHAR | Fire name |
| state_fips | VARCHAR | State FIPS |
| county_fips | VARCHAR | County FIPS of origin |
| year | INTEGER | Fire year |
| discovery_date | DATE | Date fire discovered |
| containment_date | DATE | Date fire contained (NULL if not) |
| cause | VARCHAR | Lightning / Human / Undetermined |
| acres_burned | DOUBLE | Total acres burned |
| structures_destroyed | INTEGER | Structures destroyed |
| structures_damaged | INTEGER | Structures damaged |
| fatalities | INTEGER | Fatalities |
| suppression_cost | BIGINT | Estimated suppression cost (USD) |
| latitude | DOUBLE | Fire origin latitude |
| longitude | DOUBLE | Fire origin longitude |
...
| 3 — Storm Events | `storm_events` | none | SEC risk, crop insurance, FEMA |
| 4 — Drought Monitor | `drought_monitor_weekly` | none | Agriculture, water, migration |
| 5 — Wildfire/Smoke | `nifc_fire_incidents`, `hms_smoke_daily` | none | Health, timber, insurance |
| 6 — Climate Normals | `climate_normals_monthly` | none | Anomaly detection baseline |

All sources are freely available without API keys or authentication. Total new tables: 7.
...
geo.counties (county_fips)
    ├── storm_events (county_fips, begin_date)
    ├── drought_monitor_weekly (county_fips, week_date)
    ├── nifc_fire_incidents (county_fips, discovery_date)
