...
| `energy.eia_state_energy_consumption` | **REPARTITIONED** ✓ | `(type,year)`→`(type)` | One cumulative SEDS.zip (all years, transformer emits year); 55 MB → 1 file (was 1,714). Inlined `[type]` partition. Lake (1,984)+R2 purged. |
| `energy.eia_natural_gas_storage` | **REPARTITIONED** ✓ | `(type,year)`→`(type)` | Same single-cumulative-fetch pattern; 0.4 MB → 1 file (was 69, worst file/byte ratio). Lake+R2 purged. |
